...
def generate_headers(ctx: PlatformCTX) -> Dict[str, str]:
    ...
    return {
        'Authorization': f'Bearer {ctx.args["token"]}'
    }
...
class RCTF(PlatformABC):
    ...
    async def match_platform(cls, ctx: PlatformCTX) -> bool:
        ...
        async with aiohttp.request(
                method="get", url=f"{ctx.url_stripped}/api/v1/leaderboard/now?limit=0&offset=0"
        ) as response:
    ...
    async def login(cls, ctx: PlatformCTX) -> Optional[Session]:
        ...
        async with aiohttp.request(
            ...
            # Saving token
            return Session(
                token=response_json['data']['authToken']
            )
    ...
    async def submit_flag(cls, ctx: PlatformCTX, challenge_id: str, flag: str) -> Optional[SubmittedFlag]:
        ...
        async with aiohttp.request(
            ...
            await result.update_first_blood(ctx, cls.get_challenge, challenge_id, lambda x: x <= 0)
    ...
    async def pull_challenges(cls, ctx: PlatformCTX) -> Generator[Challenge, None, None]:
        ...
        async with aiohttp.request(
                method="get", url=f"{ctx.url_stripped}/api/v1/challs", headers=generate_headers(ctx)
        ) as response:
            # Validating request
    ...
    async def pull_scoreboard(cls, ctx: PlatformCTX, max_entries_count: int = 20) -> Generator[Team, None, None]:
        ...
        async with aiohttp.request(
                method="get",
                url=f"{ctx.url_stripped}/api/v1/leaderboard/now?limit={max_entries_count}&offset=0",
                headers=generate_headers(ctx)
        ) as response:
            # Validating request
    ...
    async def get_me(cls, ctx: PlatformCTX) -> Optional[Team]:
        ...
        async with aiohttp.request(
                method="get", url=f"{ctx.url_stripped}/api/v1/users/me", headers=generate_headers(ctx)
        ) as response:
            # Validating request
    ...
    async def register(cls, ctx: PlatformCTX) -> RegistrationStatus:
        ...
        async with aiohttp.request(
            ...
            # Validating response
            if not await validate_response(response, 'kind', 'data'):
                return RegistrationStatus(success=False, message='Got an invalid response from rCTF register endpoint')
            ...
            if kind in errors:
                return RegistrationStatus(success=False, message=errors[response_json['kind']])

            # Building result object
            result = RegistrationStatus(success=True, message=response_json.get('message', 'No message'))
