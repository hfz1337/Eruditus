...
from integrations.ctftime.utils import ctftime_date_to_datetime
...
async def scrape_event_info(event_id: int) -> Optional[dict]:
    ...
    # Extract required elements with null checks
    name_elem = parser.find("h2")
    location_elem = parser.select_one("p b")
    logo_elem = parser.select_one(".span2 img")
    if not name_elem or not location_elem or not logo_elem:
        return None

    event_name = name_elem.text.strip()
    event_location = location_elem.text.strip()
    event_logo = logo_elem.get("src", "").lstrip("/")
...
async def scrape_current_events() -> AsyncGenerator[dict, None]:
    ...
    event_ids = []
    for event in parser.select("td span+ a"):
        href = event.get("href", "")
        if href and "/" in href:
            try:
                event_ids.append(int(href.split("/")[-1]))
            except ValueError:
                continue
