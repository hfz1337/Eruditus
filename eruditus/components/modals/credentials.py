...
from platforms import Platform, PlatformABC, PlatformCTX
from utils.discord import make_form_field_config, update_credentials
...
async def add_credentials_callback(
    ...
    msg = f"{Emojis.SUCCESS} Credentials added."

    # Try to authorize.
    if self.platform is not None and self.platform.impl is not None:
        ctx = PlatformCTX.from_credentials(credentials)
        session = await self.platform.impl.login(ctx)

        if not session or not session.validate():
            await send_response(
                interaction, f"{Emojis.ERROR} Unable to authorize on the platform."
            )
            return

        me = await self.platform.impl.get_me(ctx)
        if me:
...
async def register_account_callback(
    ...
    match Platform(self.platform):
        case Platform.RCTF:
            ...
            result = await self.platform.impl.register(ctx)
            if not result.success:
                await send_response(interaction, result.message)
                return
    ...
    await update_credentials(interaction, credentials)
    await send_response(
        interaction, result.message or f"{Emojis.SUCCESS} Registration successful."
    )
